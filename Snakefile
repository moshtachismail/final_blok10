rule all:  
	input: "output/Regression2Analysis"
...
#SNP's
rule get_snps:
	input: "inputs/SNPS_input/chr3_10_protein.txt"
	output: "output/SNP_output/get_snps/newallsnps.tsv"
	shell: "python3 src/SNP_data.py {input} {output}"

rule snp_sel:
	input: een="output/SNP_output/get_snps/newallsnps.tsv",
...
rule regressie: 
	...
	output:	directory("output/Regression2Analysis")
	shell: "mkdir {output} && python3 src/regressie_analysis_final.py {input.counts} {input.sel} {output}"
